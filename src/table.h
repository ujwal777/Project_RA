...
public:
     ...
     */
    template <typename T>
    void writeRows(vector<vector<T> > row, ostream &fout)
    {

        logger.log("Table::printRow");
        for(int i=0;i<row.size();i++){
            for (int columnCounter = 0; columnCounter < row[i].size(); columnCounter++)
            {
                if (columnCounter != 0)
                    fout << ",";
                fout << row[i][columnCounter];
            }
             fout << endl;
        }
    }
    ...
    void writeRow(vector<T> row)
    {
     ...
     template <typename T>
     void writeRows(vector<vector<T> >  row)
    {
        logger.log("Table::printRow");
        ofstream fout(this->sourceFileName, ios::app);
        this->writeRows(row, fout);
        fout.close();
    }
